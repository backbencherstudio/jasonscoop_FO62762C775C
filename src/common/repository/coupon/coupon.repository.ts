  ...
  static async applyCoupon({
    ...
    try {
      ...
      if (coupon.max_uses != null) {
        ...
        // check total usages for order coupon
        const total_usages = await prisma.orderCoupon.count({
          where: {
            coupon_id: coupon.id,
          },
        });
      ...
      if (coupon.max_uses_per_user != null) {
        ...
        // check for order coupon
        const user_usages = await prisma.orderCoupon.count({
          where: {
            coupon_id: coupon.id,
            user_id: user_id,
          },
      ...
      if (coupon.min_type == 'amount') {
        ...
        const total_amount = product_data.price;
